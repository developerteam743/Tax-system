...
interface PartyMasterManagerProps { isOpen: boolean; onClose: () => void; }
type FormState = Omit<Party, 'id' | 'currentBalance'>;
const emptyForm: FormState = { name: '', gstin: '', phone: '', email: '', address: '', city: '', state: 'Gujarat', stateCode: '24', type: 'CUSTOMER', openingBalance: 0 };
const readParties = (): Party[] => { try { const raw = localStorage.getItem('taxflow_parties'); return raw ? JSON.parse(raw) : []; } catch { return []; } };
...
export const PartyMasterManager: React.FC<PartyMasterManagerProps> = ({ isOpen, onClose }) => {
  ...
  const save = (event: React.FormEvent) => {
    ...
    if (!name) return;
    if (editingId) {
      persist(parties.map((party) => party.id === editingId ? { ...party, ...form, name, gstin: form.gstin.trim().toUpperCase() } : party));
    } else {
      const party: Party = { ...form, id: `party-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name, gstin: form.gstin.trim().toUpperCase(), currentBalance: Number(form.openingBalance) || 0 };
      persist([party, ...parties]);
    }
    setEditingId(null); setForm(emptyForm); setFormOpen(false); window.location.reload();
  };

  const remove = (party: Party) => { if (!window.confirm(`Delete ${party.name} from Customer & Vendor Master?`)) return; persist(parties.filter((item) => item.id !== party.id)); window.location.reload(); };
      ...
      <div className="w-full max-w-6xl max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)] overflow-hidden rounded-2xl bg-white shadow-2xl border border-slate-200 flex flex-col">
        ...
        <div className="p-3 sm:p-5 overflow-y-auto min-h-0">
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="w-full min-w-[780px] text-left text-xs"><thead className="bg-slate-50 text-slate-600"><tr><th className="p-3">Party</th><th className="p-3">GSTIN</th><th className="p-3">Type</th><th className="p-3">Location</th><th className="p-3 text-right">Balance</th><th className="p-3 text-right">Actions</th></tr></thead>
              <tbody className="divide-y divide-slate-100">{filtered.map((party) => <tr key={party.id} className="hover:bg-slate-50"><td className="p-3"><div className="font-bold text-slate-800">{party.name}</div><div className="text-[10px] text-slate-500">{party.phone || party.email || 'No contact'}</div></td><td className="p-3 font-mono text-slate-500">{party.gstin || 'UNREGISTERED'}</td><td className="p-3 font-bold">{party.type}</td><td className="p-3 text-slate-600">{party.city || '-'}, {party.state || '-'} ({party.stateCode || '-'})</td><td className="p-3 text-right font-mono font-bold">₹{Number(party.currentBalance || 0).toLocaleString('en-IN')}</td><td className="p-3"><div className="flex justify-end gap-2"><button type="button" onClick={() => startEdit(party)} className="inline-flex items-center gap-1 rounded-lg bg-blue-50 text-blue-700 px-2.5 py-2 font-bold"><Pencil className="w-3.5 h-3.5" /> Edit</button><button type="button" onClick={() => remove(party)} className="inline-flex items-center gap-1 rounded-lg bg-red-50 text-red-700 px-2.5 py-2 font-bold"><Trash2 className="w-3.5 h-3.5" /> Delete</button></div></td></tr>)}</tbody>
            </table>
